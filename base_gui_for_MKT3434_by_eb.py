...
####
    def handle_missing_data(self):
        ...
        elif method in ["Interpolation", "Forward Fill", "Backward Fill"]:
            fill_method = 'ffill' if method == "Forward Fill" else 'bfill'
            self.X_train = pd.DataFrame(self.X_train).fillna(method=fill_method).values
            self.X_test = pd.DataFrame(self.X_test).fillna(method=fill_method).values
####
    ...
    def train_model(self, model_name, param_widgets):
        """Train the selected machine learning model with user-defined parameters"""
        try:
            # Get parameters from UI
            params = {param: widget.value() if isinstance(widget, (QSpinBox, QDoubleSpinBox))
            else widget.isChecked() if isinstance(widget, QCheckBox)
            ...
            if model_name == "Support Vector Machine":
                ...
                C = max(params.get("C", 1.0), 0.1)  # Ensure C is at least 0.1
                kernel = params.get("kernel", "rbf")
                degree = params.get("degree", 3)
                epsilon = params.get("epsilon", 0.1)
                loss = params.get("loss (Classification)", "hinge")

                # Determine classification vs. regression
                if len(set(self.y_train)) > 10:  # Regression (SVR)
                    self.current_model = SVR(kernel=kernel, C=C, epsilon=epsilon, degree=degree)
                else:  # Classification (SVC)
                    self.current_model = SVC(kernel=kernel, C=C, degree=degree, probability=True)

            # Train the model
            self.current_model.fit(self.X_train, self.y_train)
            predictions = self.current_model.predict(self.X_test)

            # Evaluate and display results
            self.evaluate_model(predictions, self.y_test)
...
# preprocessing data
    def preprocess_data(self, X_train, X_test):
        """Preprocess data (e.g., convert categorical columns to numeric, handle missing values)"""
        ...
        # If you have any categorical columns, apply LabelEncoder
        # This part should be modified based on your specific dataset
        label_encoder = LabelEncoder()

        # Assuming the dataset contains categorical features that need encoding
        for col in range(X_train.shape[1]):
            if isinstance(X_train[0, col], str):  # Check if the column is categorical
    ...
    def evaluate_model(self, predictions, y_test):
        """Evaluate the trained model and display results in the status bar"""
        try:
            from sklearn.metrics import accuracy_score, mean_squared_error, mean_absolute_error

            if len(set(y_test)) > 10:  # Regression
                mse = mean_squared_error(y_test, predictions)
                mae = mean_absolute_error(y_test, predictions)
                self.status_bar.showMessage(f"Regression Results: MSE={mse:.4f}, MAE={mae:.4f}")
            else:  # Classification
                acc = accuracy_score(y_test, predictions)
                self.status_bar.showMessage(f"Classification Accuracy: {acc:.4%}")
